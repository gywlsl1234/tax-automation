...
- **관리자 대시보드에서 상태 가시성 확보**: `/admin/reports/[id]/preview`의
  ...
  전부 볼 수 있다.
- **모바일 열람 제한은 보안 통제가 아니다.** `/r/[token]`은 요청 헤더의
  User-Agent가 모바일 기기로 보이면 보고서 대신 안내 화면을 보여준다
  (`src/lib/publicLink/device.ts`). User-Agent는 클라이언트가 얼마든지
  조작할 수 있는 값이므로, 이 검사는 접근 제어가 아니라 "고객이 PC에서
  열람하도록 유도"하는 UX 차원의 안내일 뿐이다. 실제 데이터 보호는 여전히
  비밀번호 인증 + 세션 쿠키에 의존한다.
