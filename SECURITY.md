...
- **관리자 대시보드에서 상태 가시성 확보**: `/admin/reports/[id]/preview`의
  공유 링크 패널에서 각 링크의 사용중/만료/폐기 상태, 실패 횟수,
  발급·만료·폐기 시각을 전부 볼 수 있다.
- **공유 링크는 발급일로부터 60일이 지나면 만료된다.** `report_links.expires_at`
  은 발급 시 DB 기본값(`now() + interval '60 days'`)으로 자동 설정된다.
  `status`가 여전히 `active`여도 `expires_at`이 지났으면 `/r/[token]` 페이지와
  `/api/public/report-links/[token]/verify`가 비밀번호 검증 시도 전에 즉시
  차단한다(폐기 링크와 동일한 위치·순서 — 검증 자체를 시도하지 않는다).
- `/r/[token]`은 모바일 기기를 포함해 모든 기기의 브라우저에서 동일하게
  열람 가능하다 (기기 종류로 접근을 제한하지 않는다).
